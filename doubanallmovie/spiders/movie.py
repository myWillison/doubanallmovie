...
class MovieSpider(Spider):
    ...
    start_urls = ['https://movie.douban.com/subject/1300300/','https://movie.douban.com/subject/26861685/','https://movie.douban.com/subject/26644205/','https://movie.douban.com/subject/26684564/']#从一个电影开始
    ...
    def parse_index(self, response):
        ...
        score = response.css('#interest_sectl > div.rating_wrap.clearbox > div.rating_self.clearfix > strong::text').extract_first()
        people_num = response.css('#interest_sectl > div.rating_wrap.clearbox > div.rating_self.clearfix > div > div.rating_sum > a > span::text').extract_first()
        rate5 = response.css('#interest_sectl > div.rating_wrap.clearbox > div.ratings-on-weight > div:nth-child(1) > span.rating_per::text').extract_first()
        rate4 = response.css('#interest_sectl > div.rating_wrap.clearbox > div.ratings-on-weight > div:nth-child(2) > span.rating_per::text').extract_first()
        rate3 = response.css('#interest_sectl > div.rating_wrap.clearbox > div.ratings-on-weight > div:nth-child(3) > span.rating_per::text').extract_first()
        rate2 = response.css('#interest_sectl > div.rating_wrap.clearbox > div.ratings-on-weight > div:nth-child(4) > span.rating_per::text').extract_first()
        rate1 = response.css('#interest_sectl > div.rating_wrap.clearbox > div.ratings-on-weight > div:nth-child(5) > span.rating_per::text').extract_first()
        ...
        for start in range(0,220,20):#这是无登陆
            yield Request(url=comment_url.format(start=start,limit=20),callback=self.parse_comments,meta={'title':title})
